...
export async function GET(request: NextRequest) {
  try {
    ...
    const [jobPositionsDb, total] = await Promise.all([
      prisma.job_positions.findMany({ skip, take: pageSize, orderBy: { created_at: "desc" } }),
      prisma.job_positions.count(),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const jobPositionDb = await prisma.job_positions.create({
      data: {
        title: payload.title,
